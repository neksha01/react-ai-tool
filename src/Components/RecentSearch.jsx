function RecentSearch({ recentHistory, setRecentHistory, setSelectedHistory }) {
  ...
  return (
    <div
      className="col-span-1 dark:bg-zinc-800 bg-red-100 
                 pt-3 h-full md:h-auto 
                 w-full md:w-auto"
    >
      {/* Header */}
      <h1 className="text-lg md:text-xl 
        ...
        <button
          ...
          className="cursor-pointer p-1 md:p-2 
                     rounded-md hover:bg-zinc-900 bg-zinc-700"
        >
          <svg
      ...
      <ul className="text-left overflow-y-auto mt-2 max-h-[50vh] md:max-h-[70vh]">
        {recentHistory &&
          recentHistory.map((item, index) => (
            <div
              key={index}
              className="flex justify-between items-center pr-2 md:pr-3 py-1"
            >
              <li
                className="w-full px-3 md:px-5 
                           ...
                           hover:bg-red-200 hover:text-zinc-900 
